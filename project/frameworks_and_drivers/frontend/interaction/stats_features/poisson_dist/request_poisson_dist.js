...
export function requestPoissonDist(start, end, mean) {
    ...
    const PATH = `http://localhost:80/api/stats?stats_type=poisson_dist&start=${start}&end=${end}&mean=${mean}`;
        let statusCode;
        ...
        fetch(PATH, {
            ...
            if (statusCode === 200) {
                loadPoissonFrontend(start, end, json);
            }
...
const loadPoissonFrontend = (start, end, json) => {
